  ...
  <form @submit.prevent="filter">
    <div class="mb-8 mt-4 flex flex-wrap gap-2">
      <div class="flex flex-nowrap items-center">
        <input v-model.number="filterForm.priceFrom" type="text"
          placeholder="Price from" class="input-filter-l w-28" />
        <input v-model.number="filterForm.priceTo" type="text"
          placeholder="Price to" class="input-filter-r w-28" />
      </div>
      <div class="flex flex-nowrap items-center">
...
const filterForm = useForm({
  ...
  areaTo: props.filters.areaFrom ?? null,
});

const filter = () => {
  filterForm.get(
    route('listing.index'), { preserveScroll: true, preserveState: true }
  )
};
